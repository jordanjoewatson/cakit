...
class CakitTableView(QTableView):

    table_record_deleted = pyqtSignal(Record)
    signal_send_to_decrypt = pyqtSignal(Record)

    def __init__(self, cakit_table_model):
        super().__init__()
        self.model = cakit_table_model
        header = self.horizontalHeader()
        header.setSectionResizeMode(QHeaderView.ResizeMode.Stretch)
        # Define headers and initial data
        # headers = ["a", "bsbs"]
